...
import static com.slprime.chromatictooltips.util.NumberFormatUtil.BD_THOUSAND;
import static com.slprime.chromatictooltips.util.NumberFormatUtil.formatNumber;
...
public enum ExponentialFormat {
    ...
    public static ExponentialFormat parse(String raw) {
        try {
            return ExponentialFormat.valueOf(
                raw.trim()
    ...
    private static String formatScientificEngineering(BigDecimal value, NumberOptionsBase<?> options) {
        ...
        if (absMantissa.compareTo(BD_THOUSAND) >= 0) {
            mantissa = mantissa.movePointLeft(3)
                .setScale(options.getDecimalPlaces(), options.getRoundingMode());
            engExp += 3;
        }
